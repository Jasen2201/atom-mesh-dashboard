#!/usr/bin/env python3
"""One-shot: pull DeepSeek-R1 MI355X mori-sglang reference data from
the public InferenceX API and write data/infx_*.json + refresh data/index.json.

All configs for a given (precision, ISL, OSL) are merged into ONE run file so
the dashboard draws a single combined Pareto curve per precision — matching
InferenceX's own "Token Throughput per GPU vs. Interactivity" presentation.
...
def to_runs(rows):
    ...
    for precision in PRECISIONS:
        ...
        print(f"{len(matches)} rows for precision={precision} "
              f"(hardware={HARDWARE}, framework={FRAMEWORK}, "
